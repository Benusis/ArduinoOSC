// This is version 1.02(2021.01.06)
// Tested in Unity 2020
// Most of the code is based on a library for the Make Controller Kit1
// Modified By vincent MEYRUEIS (ENSAM-LCPI) Paris 
...
public class UDPPacketIO 
{
	...
	public int ReceivePacket(byte[] buffer)
	{
        ...
        RemoteHostName = iep.Address.ToString();//DEBUG MODIFICATION
        RemotePort = iep.Port;
...
//[DEBUG]
/// <summary>
/// The OscBundle class is a data structure that represents
/// an OSC address and an arbitrary number of Osc Message
/// </summary>
...
public class OscMessage
  {
   ...
   public string address;
    ...
    /// <summary>
    /// The Host Name of the message as a string.
    /// </summary>
    public string RemoteMessageHostName;

    /// <summary>
    /// The Host Name of the message as a string.
    /// </summary>
    public int RemoteMessageHostPort;
  ...
  public class OSC : MonoBehaviour
  {
	...
	void Awake() {
        ...
        if (HostName != "")
        {
            ...
            if (ipHostEntry != null)
            {
                outIP = ipHostEntry.AddressList[0].ToString();
            }
        ...
        UnityEditor.EditorApplication.playModeStateChanged += x => { HandleOnPlayModeChanged(); };
    #endif
    ...
    */
				...
				if (length > 0)
				{
					lock(ReadThreadLock) {
						...
						if ( paused == false ) {
                            ...
                            {
                                ((OscMessage)newMessages[i]).RemoteMessageHostName = OscPacketIO.RemoteHostName;
                                ((OscMessage)newMessages[i]).RemoteMessageHostPort = OscPacketIO.RemotePort;
                            }
    ...
    public void Send( OscMessage oscMessage )
    {
      ...
      int length = OSC.OscMessageToPacket( oscMessage, packet, 10000 );
    ...
    {
      byte[] packet = new byte[10000];
		int length = OSC.OscMessagesToPacket(oms, packet, 10000);
      OscPacketIO.SendPacket(packet, length);
    }
    ...
    //[TODO] Test Time Stamp
    //private static int ExtractMessages(ArrayList messages, byte[] packet, int start, int length, byte[] TimeStamp)
        ...
        case '#':
          ...
          if ( bundleString == "#bundle" )
          {
           ...
           index += 16;
            ...
            /*
            //[TODO/] Get TimeStamp sur les 8 bit 
            index +=8;
            for (int i = 0; i < 8; i++) {
              TimeStamp[i] = packet[index++];
            }
            */
      ...
      foreach (char c in typeTag)
      {
            switch (c)
            {
                ...
                case 'f': //Float
                    ...
                    /* [TODO] TimeStamp Managment  
                    case 't': //TimeStamp NTP
                      {
